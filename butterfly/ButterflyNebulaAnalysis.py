...
image_data_52_cont = np.zeros((len(image_data_57[:,0,0]), array_length_x, array_length_y))

points52 = [5.15, 5.39, 5.55, 5.81] #used to be [5.06, 5.15, 5.39, 5.55]

for i in range(array_length_x):
    for j in range(array_length_y):
        image_data_52_cont[:,i,j] = bnf.linear_continuum_single_channel(
            wavelengths57, image_data_57[:,i,j], points52)
    ...
    for j in range(array_length_y):
        if region_indicator[i,j] == 1:
            pah_intensity_52[i,j] = bnf.pah_feature_integrator(wavelengths1a[lower_index_52:upper_index_52], 
                                                            image_data_57[lower_index_52:upper_index_52,i,j] - image_data_52_cont[lower_index_52:upper_index_52,i,j])
...
for i in range(array_length_x):
    for j in range(array_length_y):
        if region_indicator[i,j] == 1:
            pah_intensity_error_52[i,j] = bnf.error_finder(wavelengths57, image_data_57[:,i,j] - image_data_52_cont[:,i,j], 5.25, 
...
for i in range(array_length_x):
    for j in range(array_length_y):
        image_data_3c_cont[:,i,j] = bnf.linear_continuum_single_channel(
