...
# idea: a single class, that when called will reproject data A onto data B, and save a new version of data A.
# the old code worked with only 3 arguments, so this should be good for this code also

# make an option to do only data, or spend time on the error and dq arrays.

# takes so long to run, this should also be a 'run on its own' thing

# whats it need to be, at a bare mininum? 

# can the WCS to header and back be skipped? also figure out what exactly WCS uses to
# get the coords

# an entire function dedicated to removing negatives and infs. Infs should go, 
# negatives may be 0 + noise and so should stay

# figure out what find_optimal_celestial_wcs does, and if it accounts for rotation

# should i use reproject exact or some other function? also what does parallel do

# does the data really need to be in a weird order? if yes, do this first for efficiency

# will need to change a lot more in the header, for example rotation and the matrix
